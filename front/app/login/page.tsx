...
export default function LoginPage() {
    ...
    const handleLogin = async ( formData : FormData) => {
        ...
        try{
            const loginData: LoginResponse = await loginService({ email, senha })
            console.log(loginData)

            dispatch(login({ usuario: loginData.usuario, token: loginData.token}))
            if(loginData.usuario.role === "ADMINISTRADOR"){
                router.push("/admin")
            } else {
                router.push("/home")
            }
