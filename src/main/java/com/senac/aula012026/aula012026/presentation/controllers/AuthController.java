...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest){
        ...
        if(usuarioService.ValidaUsuarioSenha(loginRequest)){
            ...
            var usuario = usuarioRepository.findByEmail(loginRequest.email()).orElseThrow();

            return ResponseEntity.ok(new LoginResponse(token, new UsuarioLogadoResponse(usuario)));
        }
